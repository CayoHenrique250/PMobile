...
class DBHelper {
  ...
  Future<FutureOr<void>> onCreate(Database db, int version) async {
    ...
    String sql =
        'CREATE TABLE IF NOT EXISTS user (id CHAR PRIMARY KEY, nome varchar(100), sobrenome varchar(100), email varchar(100), senha varchar(100), ocupacao varchar(100));';
    ...
    sql =
        'CREATE TABLE IF NOT EXISTS SOBRE (id INTEGER AUTO_INCREMENT PRIMARY KEY, imagem varchar(100), titulo varchar(100), descricao varchar(100), cor varchar(20), cor_titulo varchar(20));';
    await db.execute(sql);

    sql =
        "INSERT INTO SOBRE (imagem, titulo, descricao, cor, cor_titulo) VALUES ('assets/imagens/sustentabilidade.jpg', 'Sustentabilidade', 'Sustentabilidade é a capacidade de cumprir com as necessidades do presente sem comprometer as mesmas das gerações futuras.', '0xFFFFFFFF', '0xFF1B5E20');";
    await db.execute(sql);

    sql =
        "INSERT INTO SOBRE (imagem, titulo, descricao, cor, cor_titulo) VALUES ('assets/imagens/meioAmbiente.jpg', 'Meio Ambiente', 'O meio ambiente diz respeito ao conjunto de elementos e processos biológicos, químicos e físicos responsáveis pela vida no planeta Terra.', '0xFFCCFF90', '0xFF1B5E20');";
    await db.execute(sql);

    sql =
        "INSERT INTO SOBRE (imagem, titulo, descricao, cor, cor_titulo) VALUES ('assets/imagens/tsma.jpg', 'Projeto TSMA', 'Equipe:\n\nAmanda Fernandes;\nCayo Henrique;\nNatally Emanuelle e\nSilmara Nunes.', '0xFFFFFFFF', '0xFF1B5E20');";
    await db.execute(sql);
    ...
    sql =
        'CREATE TABLE IF NOT EXISTS TUTORIAIS (id INTEGER AUTO_INCREMENT PRIMARY KEY, imagem varchar(100), titulo varchar(100), cor varchar(20), cor_titulo varchar(20), descricao varchar(100), proxima varchar(20));';
    await db.execute(sql);

    sql =
        "INSERT INTO TUTORIAIS (imagem, titulo, cor, cor_titulo, descricao) VALUES ('assets/imagens/tartarugaPet.jpg', 'Tartarugas PET', '0xFFFFFFFF', '0xFF1B5E20', 'Tartarugas feitas com fundo de garrafa PET e EVA.');";
    await db.execute(sql);

    sql =
        "INSERT INTO TUTORIAIS (imagem, titulo, cor, cor_titulo, descricao) VALUES ('assets/imagens/porcoEspinho.jpg', 'Porco espinho de revista', '0xFFCCFF90', '0xFF1B5E20', 'Porco espinho de papelão e recortes de revistas.');";
    await db.execute(sql);
    ...
    sql =
        'CREATE TABLE IF NOT EXISTS PORTALCARD (ID INT AUTO_INCREMENT PRIMARY KEY, IMAGEM VARCHAR(100));';
    await db.execute(sql);
    ...
    sql =
        'CREATE TABLE IF NOT EXISTS PORTAL (ID INT AUTO_INCREMENT PRIMARY KEY, URL VARCHAR(100), TITULO VARCHAR(50), ID_PORTALCARD INTEGER, FOREIGN KEY (ID_PORTALCARD) REFERENCES PORTALCARD (ID));';
    await db.execute(sql);
    ...
    await db.execute(sql);
     ...
     sql =
        'CREATE TABLE IF NOT EXISTS TUTORIAL1 (ID INT AUTO_INCREMENT PRIMARY KEY, VIDEO VARCHAR(100), TITULO VARCHAR(100), TEXTO VARCHAR(200), TITULO2 VARCHAR(100));';
    await db.execute(sql);
    ...
    await db.execute(sql);
     ...
     sql =
        'CREATE TABLE IF NOT EXISTS TEXTO_IMG (ID INT AUTO_INCREMENT PRIMARY KEY, TEXTO VARCHAR(200), IMAGEM VARCHAR(100), ID_TUTORIAL1 INTEGER, FOREIGN KEY (ID_TUTORIAL1) REFERENCES TUTORIAL1 (ID));';
    await db.execute(sql);
    ...
    await db.execute(sql);
     ...
     sql =
        'CREATE TABLE IF NOT EXISTS TUTORIAL2 (ID INT AUTO_INCREMENT PRIMARY KEY, VIDEO VARCHAR(100), TITULO VARCHAR(100), TEXTO VARCHAR(200), TITULO2 VARCHAR(100));';
    await db.execute(sql);
    ...
    sql =
        'CREATE TABLE IF NOT EXISTS TEXTO_IMG2 (ID INT AUTO_INCREMENT PRIMARY KEY, TEXTO VARCHAR(200), IMAGEM VARCHAR(100), ID_TUTORIAL1 INTEGER, FOREIGN KEY (ID_TUTORIAL1) REFERENCES TUTORIAL1 (ID));';
    await db.execute(sql);
